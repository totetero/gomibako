...
class Character{
	...
	function init(img : HTMLImageElement) : void {
		this.img = img;
		for(var i = 0; i < 10; i++){this.plist[i] = new Parts();}
	}
	...
	function pushParts(p : Parts, type : int, x : number, y : number, z : number) : void{
		...
		switch(type){
			...
			case 7: p.s = 16; p.u =  0 + this.ppd * p.s; p.v = 80; break;
		}
	}
		...
		if(this.action > 0){
			switch(Math.floor(this.action / 6) % 4){
				case 0:
					...
					this.pushParts(this.plist[8], 6, -0.07,  0.00, 0.36);
					this.pushParts(this.plist[9], 7, -0.04, -0.02, 0.53);break;
				case 1:
					...
					this.pushParts(this.plist[8], 6, -0.07,  0.00, 0.38);
					this.pushParts(this.plist[9], 7, -0.04,  0.00, 0.55);break;
				case 2:
					this.pushParts(this.plist[0], 0,  0.12,  0.00, 0.45);
					this.pushParts(this.plist[1], 1,  0.00,  0.00, 0.23);
			...
			this.pushParts(this.plist[0], 0,  0.00, -0.03, 0.52);
			this.pushParts(this.plist[1], 1, -0.02, -0.02, 0.27);
			...
			this.pushParts(this.plist[5], 4, -0.02,  0.18, 0.45);
			this.pushParts(this.plist[6], 5, -0.05,  0.17, 0.50);
			this.pushParts(this.plist[7], 5, -0.05, -0.23, 0.50);
			this.pushParts(this.plist[8], 6, -0.15, -0.03, 0.40);
			this.pushParts(this.plist[9], 7, -0.16,  0.00, 0.60);
		}else{
