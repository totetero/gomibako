...
function Engine3d(){
	...
	this.initTemplate = function(){
		...
		this.sphereTexture = e3d.createTexture(canvas);
		var size = 32;
		var vert = new Array();
		var texc = new Array();
		var face = new Array();
		for(var i = 0; i < size; i++){
			for(var j = 0; j < size; j++){
				vert.push(0.5 * Math.sin(Math.PI * i / (size - 1)) * Math.cos(Math.PI * 2 * j / (size - 1)));
				vert.push(0.5 * Math.cos(Math.PI * i / (size - 1)));
				vert.push(0.5 * Math.sin(Math.PI * i / (size - 1)) * Math.sin(Math.PI * 2 * j / (size - 1)));
				texc.push(1.0 - j / (size - 1));
				texc.push(1.0 - i / (size - 1));
			}
		}
		for(var i = 0; i < size - 1; i++){
			for(var j = 0; j < size; j++){
				face.push(size * (i + 1) + (j + 0));
				face.push(size * (i + 0) + (j + 0));
				face.push(size * (i + 1) + (j + 1) % size);
				face.push(size * (i + 0) + (j + 1) % size);
				face.push(size * (i + 1) + (j + 1) % size);
				face.push(size * (i + 0) + (j + 0));
			}
		}
		this.sphereVertBuffer = e3d.createVBO(vert);
		this.sphereTexcBuffer = e3d.createVBO(texc);
		this.sphereFaceBuffer = e3d.createIBO(face);
		this.sphereFaceNum = face.length;
		this.drawSphere = function(mat, x, y, z, size){
			mat4.set(this.worldmat, this.tmpmat1);
			mat4.translate(this.tmpmat1, [x, z, y]);
			mat4.scale(this.tmpmat1, [size, size, size]);
			this.setMatrix(this.tmpmat1);
			this.bindTex(this.sphereTexture, 1);
			this.bindVertBuf(this.sphereVertBuffer);
			this.bindTexcBuf(this.sphereTexcBuffer);
			this.bindFaceBuf(this.sphereFaceBuffer);
			gl.drawElements(gl.POINTS, this.sphereFaceNum, gl.UNSIGNED_SHORT, 0);
		}
	}
