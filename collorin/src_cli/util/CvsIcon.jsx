...
class CvsIcon{
	...
	// ----------------------------------------------------------------
	// canvasをbase64に変換 
	static function toDataURL(canvas : HTMLCanvasElement) : string{
		// 画像ピクセルの取得
		...
		var uint8Array = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a];

		var toUint = function(num : int) : int{return num < 0 ? num + 4294967296 : num;};
		var pushBytes32 = function(num : int) : void{uint8Array.push((num >>> 24) & 0xff, (num >>> 16) & 0xff, (num >>> 8) & 0xff, num & 0xff);};
		var pushBytes16sw = function(num : int) : void{uint8Array.push(num & 0xff, (num >>> 8) & 0xff);};
		var pushCrc32 = function(start : int, length : int) : void{
			...
			var temp = 0xffffffff;
			for(var i = 0; i < length; i++){temp = CvsIcon.crc32table[(temp ^ uint8Array[start + i]) & 0xff] ^ (temp >>> 8);}
			pushBytes32(toUint(temp ^ 0xffffffff));
		};

		// イメージヘッダ
		uint8Array.push(0x00, 0x00, 0x00, 0x0d);
		var crcStart = uint8Array.length;
		uint8Array.push(0x49, 0x48, 0x44, 0x52);
		pushBytes32(canvas.width);
		pushBytes32(canvas.height);
		uint8Array.push(0x08, 0x06, 0x00, 0x00, 0x00);
		var crcLength = uint8Array.length - crcStart;
		...
		var crcStart = uint8Array.length;
		uint8Array.push(0x49, 0x44, 0x41, 0x54, 0x78, 0x01);
		for(var i = 0; i < blocks; i++){
			var blockLen = Math.min(32768, imgData.length - (i * 32768));
			uint8Array.push((i == (blocks - 1)) ? 0x01 : 0x00);
			pushBytes16sw(blockLen);
			pushBytes16sw(~blockLen);
			for(var j = 0; j < blockLen; j++){
				uint8Array.push(imgData[32768 * i + j]);
			}
		}
		var lowerBytes = 1;
		var upperBytes = 0;
		...
		// イメージ終端
		uint8Array.push(0x00, 0x00, 0x00, 0x00);
		var crcStart = uint8Array.length;
		uint8Array.push(0x49, 0x45, 0x4e, 0x44);
		var crcLength = uint8Array.length - crcStart;
		pushCrc32(crcStart, crcLength);
		...
		return "data:image/png;base64," + dom.window.btoa(data);
	}
}
