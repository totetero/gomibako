...
class Util{
	...
	// ----------------------------------------------------------------
	// canvasをbase64に変換 
	static var _crc32table : int[];
	static function cvsToBase64(canvas : HTMLCanvasElement) : string{
		...
		var pushCrc32 = function(start : int, length : int) : void{
			if(Util._crc32table == null){
				Util._crc32table = new int[];
				for(var i = 0; i < 256; i++){
					var temp = i;
					for(var j = 0; j < 8; j++){temp = (temp & 1) ? (0xedb88320 ^ (temp >>> 1)) : (temp >>> 1);}
					Util._crc32table[i] = toUint(temp);
				}
			}
			var temp = 0xffffffff;
			for(var i = 0; i < length; i++){temp = Util._crc32table[(temp ^ charData[start + i]) & 0xff] ^ (temp >>> 8);}
