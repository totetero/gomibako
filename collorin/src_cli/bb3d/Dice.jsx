...
class DrawDice{
	...
	function constructor(size : number){
		this._img = Loader.imgs["dot_player0"]; // TODO
		// オフスクリーンcanvasの準備
		...
		for(var i = 0; i < this._pos0.length; i++){
			for(var j = 0; j < this._pos0[i].length; j++){
				this._pos0[i][j] = (this._pos0[i][j] - 0.5) / (1 + s * 2);
			}
		}
		// 座標変換後用の配列を作成
	...
	function draw(ccvs : Ccvs) : void{
		...
		ccvs.context.arc(0, 0, this._size * 0.65, 0, Math.PI*2, false);
		ccvs.context.fill();
