...
function gameSocketInit(game){
	...
	game.socket.gameRoom.addEventListener('p', function(data){
		...
		gameStruct.netPlayer.action = data.a;
		gameStruct.netPlayer.rotate = data.r;
		gameStruct.netPlayer.sradius = data.s;
	});
...
function gameSocketPush_ppos(){
	if(gameStruct.gameSocketConnected){
		...
		var xx = gameStruct.player.x;
		var yy = gameStruct.player.y;
		var zz = gameStruct.player.z;
		var aa = gameStruct.player.action;
		var rr = gameStruct.player.rotate;
		var ss = gameStruct.player.sradius;
		game.socket.gameRoom.push('p',{x:xx, y:yy, z:zz, a:aa, r:rr, s:ss});
	}
}
...
function toggleGameScene(game){
	...
	sprite.addEventListener('enterframe', function(e){
		...
		mulMat44Translate(worldMat, tmpMat1, -gameStruct.player.x, 0, -gameStruct.player.y);
	...
	group.addEventListener('enterframe', function(e){
		...
		if(gameStruct.time > gameStruct.finishTime + 8 * 3){
			...
			if(id == null){
				...
				// ハイスコア登録
				// ランキングはダメージ率で計算する
				// だってみんな僕のゲームで真面目に高得点狙おうとしないでしょ
				// ダメージを貯めたまま生き残る方が実は難易度は高いし
				location.replace([
					'http://9leap.net/games/', id[1], '/result',
					'?score=', encodeURIComponent(gameStruct.damage),
					'&result=', encodeURIComponent(gameStruct.score + "点ブレイク!! " + gameStruct.damage + "%ダメージ!!")
				].join(''));
			}
		}
...
function Player(img, x, y, z){
	...
	this.calc = function(){
		var act = this.action % 16;
		var mode = Math.floor(this.action / 16);
		// 速度計算
		if(!this.ground){
			// 空中にいる
			mode = 3;
			...
			gameStruct.score += 1;
			gameStruct.damage += 1;
			var sx = this.x + (0.3 + this.sradius) * Math.cos(this.rotate);
			var sy = this.y + (0.3 + this.sradius) * Math.sin(this.rotate);
			pushShot(sx, sy, this.rotate, this.sradius);
			gameSocketPush_shot(sx, sy, this.rotate, this.sradius);
			// 速度変更
			this.vx = -vec * Math.cos(this.rotate);
			this.vy = -vec * Math.sin(this.rotate);
			this.vz = vec;
		}else if((i = isHitShot(this.x, this.y, this.radius)) >= 0){
			// 弾があたった時
			pushEffect(3, this.x, this.y, 0);
			gameStruct.score -= 100;
			gameSocketPush_score(100);
			gameStruct.damage += 10;
			// 速度変更
			var x = this.x - gameStruct.shotPool[i].x;
			var y = this.y - gameStruct.shotPool[i].y;
			var rot = Math.atan2(y, x) + Math.PI;
			var vec = 0.1 * (1 + gameStruct.damage / 100);
			this.vx = -vec * Math.cos(rot);
			this.vy = -vec * Math.sin(rot);
			this.vz = 0.2;
		}else if((i = isHitEnemy(this.x, this.y, this.radius)) >= 0){
			// 敵があたった時
			pushEffect(3, this.x, this.y, 0);
			gameStruct.score -= 100;
			gameStruct.damage += 10;
			// 速度変更
			var x = this.x - gameStruct.enemyPool[i].x;
			var y = this.y - gameStruct.enemyPool[i].y;
			var rot = Math.atan2(y, x) + Math.PI;
			var vec = 0.1 * (1 + gameStruct.damage / 100);
			this.vx = -vec * Math.cos(rot);
			this.vy = -vec * Math.sin(rot);
			...
			// 向きを確認
			if(ctrlStruct.krt){
				if(ctrlStruct.kup){			this.rotate = 315 * Math.PI / 180 + ctrlStruct.rotv;}
				else if(ctrlStruct.kdn){	this.rotate =  45 * Math.PI / 180 + ctrlStruct.rotv;}
				else{						this.rotate =   0 * Math.PI / 180 + ctrlStruct.rotv;}
			}else if(ctrlStruct.klt){
				if(ctrlStruct.kup){			this.rotate = 225 * Math.PI / 180 + ctrlStruct.rotv;}
				else if(ctrlStruct.kdn){	this.rotate = 135 * Math.PI / 180 + ctrlStruct.rotv;}
				else{						this.rotate = 180 * Math.PI / 180 + ctrlStruct.rotv;}
			}else if(ctrlStruct.kup){		this.rotate = 270 * Math.PI / 180 + ctrlStruct.rotv;}
			else if(ctrlStruct.kdn){		this.rotate =  90 * Math.PI / 180 + ctrlStruct.rotv;}
		...
		// 接地確認
		this.ground = this.z <= 0;
		this.ground &= (0 <= this.x && this.x <= gameStruct.floorx);
		this.ground &= (0 <= this.y && this.y <= gameStruct.floory);
		if(this.ground){this.z = 0; this.vz = 0;}
		
		// 谷底確認
		if(this.z < -10){
			this.x = gameStruct.floorx / 2;
			this.y = gameStruct.floory / 2;
			this.z = 6;
			this.vx = 0;
			this.vy = 0;
			this.vz = 0;
			gameStruct.damage = 0;
			gameStruct.score = Math.floor(gameStruct.score / 2);
			gameSocketPush_score(gameStruct.score);
			this.emotion = 0;
		}
	}
	...
	this.draw = function(mat){
		...
		characterBillBoard(this.pimg, mat, this.x, this.y, this.z, this.rotate, u, size, size);
		...
		// 吹き出し描画
		emotionBillBoard(gameStruct.img6, mat, this.x, this.y, this.z + 1, this.emotion);
	}
}
...
function characterBillBoard(img, mat, x, y, z, rotate, u0, xsize, ysize){
	// 角度
	var v0 = 0;
	var angle = (rotate - ctrlStruct.rotv) / Math.PI * 180;
